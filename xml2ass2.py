...
def ReadCommentsBilibili(f, fontsize):
    ...
    for i, comment in enumerate(comment_element):
        try:
            ...
            continue
def ReadCommentsBilibili1(xml_str, fontsize):
...
def ReadCommentsBilibili2(xml_str, fontsize):
    dom = xml.dom.minidom.parseString(xml_str)
    comment_element = dom.getElementsByTagName('d')
    for i, comment in enumerate(comment_element):
        try:
...
def getComments(cid,font_size = 25):
    ...
    if '<?xml version="1.0"' in data:
        comments.extend(ReadCommentsBilibili1(data, font_size))
    elif '<?xml version="2.0"' in data:
        comments.extend(ReadCommentsBilibili2(data,font_size))
    else:
        return comments
...
def write2file(comments, stage_width, stage_height,reserve_blank=0, font_face=_('(FONT) sans-serif')[7:], font_size=25.0, text_opacity=1.0, duration_marquee=5.0, duration_still=5.0, comment_filter=None, comment_filters_file=None, is_reduce_comments=False, progress_callback=None):
    filters_regex = []
...
def xml2ass(cid, width, height, protect, font, fontsize, alpha,  duration_marquee, duration_still, filter, filter_file, reduce):
    comments = getComments(cid,fontsize)
    # print(comments)
    global contents
    contents = ""
    write2file(comments, width, height, protect, font, fontsize, alpha,  duration_marquee, duration_still, filter, filter_file, reduce)
    # print(contents)
    return contents
